  ...
  ROS_INFO_NAMED("tutorial", "End effector link: %s", move_group.getEndEffectorLink().c_str());
    ...
    while (ros::ok()){
        ...
        if (Keyboard == 32){
            ...
            target_pose1.position.z = grasp_z + 0.07 ;  // 先到预抓取位置（比抓取位置高0.05米），保证夹抓下降路径不会碰到零件
...
//            joint_group_positions[6] = M_PI/2 +  grasp_angle2;
            ...
            //-----------------------------------------------------  move franka to grasp  ------------------------------------------------------
            target_pose2.position.z = grasp_z +0.04 ;  // 到抓取位置
...
//            moveit::planning_interface::MoveGroupInterface::Plan my_plan;
            ...
            //-----------------------------------------------------  move the end up  ------------------------------------------------------
            target_pose2.position.z = grasp_z + 0.10;  // 上升0.05米，保证夹抓移动路径不会碰到零件
            move_group.setPoseTarget(target_pose2);
//            moveit::planning_interface::MoveGroupInterface::Plan my_plan;
            success = (move_group.plan(my_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
            ROS_INFO_NAMED("tutorial", "Visualizing plan 1 (pose goal) %s", success ? "" : "FAILED");
  ...
  joint_group_positions[0] = -0.047091221301836815;
  joint_group_positions[1] = 0.016230400086364203;
  joint_group_positions[2] = -0.3310777271159938;
  joint_group_positions[3] = -2.278337530314488;
  joint_group_positions[4] = -0.029808598331279225;
  joint_group_positions[5] =  2.272856513456062;
  joint_group_positions[6] =  0.512226148340776;
